 ...
 */
public final class NestedComponent extends AbstractCompilerOutput {

    private NestedComponent(CodeGeneratorResponse.File file) {
        super(file);
    }

    public static NestedComponent from(StringTypeValue generatedCode, MessageType messageType) {
        ...
        CodeGeneratorResponse.File result = file.setInsertionPoint(insertionPoint)
                                                .setContent(content)
                                                .build();
        return new NestedComponent(result);
    }
}
